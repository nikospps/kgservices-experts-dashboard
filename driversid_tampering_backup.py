...
def tampering(data,driversid_list):
    ...
    for n in range(len(data)):
        for m in range(len(driversid_list)):
            ...
            comparison_results.append(jellyfish.jaro_similarity(data[n]["Driver's personal device"]['driverID'], driversid_list[m]))
        # comparison_results.append('stop')
        for c in range(len(comparison_results)):
            if (comparison_results[c] == float(1)):
                tamperflag = False
                nontampered_sensors.append(data[n]["Driver's personal device"]['driverID'])
                # print(tamperflag, data[n]["Driver's personal device"]['driverID'])
        comparison_results = []
        tamperflag = True

    for n in range(len(data)):
        if (data[n]["Driver's personal device"]['driverID'] not in nontampered_sensors):
            tampered_sensors.append(data[n]["Driver's personal device"]['driverID'])
