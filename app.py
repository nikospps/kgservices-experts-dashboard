...
app = Flask(__name__)
CORS(app, expose_headers=['Access-Control-Allow-Origin', 'x-total-count'])
...
def meaaiso():
    ...
    return jsonify(message)
########################################################################################################################
@app.route('/vs_nkua', methods=['GET'])
@cross_origin()
def vs_nkua():
    vsm_init = get_vs.get_vs(access_token)
    nkua_vsm = []
    for n in vsm_init:
        result1 = patients_filter.patients_filter([n], patients_lists.list_nkua)
        if (result1 == True):
    ...
    for n in nkua_vsm:
        newMessage = {
            'organization': n['partner'],
            'email': n['email'],
            'source':n['source'],
            'time': n['time'],
            's_money': n['s_money'],
            's_overall': n['s_overall'],
            's_quan': n['s_quan'],
            's_time': n['s_time'],
            's_timeI': n['s_timeI'],
            's_timeS': n['s_timeS'],
    ...
    return jsonify(message)
########################################################################################################################
@app.route('/vs_fism', methods=['GET'])
@cross_origin()
def vs_fism():
    vsm_init = get_vs.get_vs(access_token)
    fism_vsm = []
    for n in vsm_init:
        result1 = patients_filter.patients_filter([n], patients_lists.list_fism)
        if (result1 == True):
            fism_vsm.append(n)
    message = []
    for n in fism_vsm:
    ...
    return jsonify(message)
########################################################################################################################
@app.route('/vs_suub', methods=['GET'])
@cross_origin()
def vs_suub():
    vsm_init = get_vs.get_vs(access_token)
    suub_vsm = []
    for n in vsm_init:
        result1 = patients_filter.patients_filter([n], patients_lists.list_suub)
        if (result1 == True):
            suub_vsm.append(n)
    message = []
    for n in suub_vsm:
    ...
    return jsonify(message)
########################################################################################################################
@app.route('/vkb_nkua', methods=['GET'])
@cross_origin()
def vkb_nkua():
    vkb_init = get_vkb.get_vkb(access_token)
    nkua_vkb = []
    for n in vkb_init:
        result1 = patients_filter.patients_filter([n], patients_lists.list_nkua)
        if (result1 == True):
    ...
    return jsonify(message)
########################################################################################################################
@app.route('/vkb_fism', methods=['GET'])
@cross_origin()
def vkb_fism():
    vkb_init = get_vkb.get_vkb(access_token)
    fism_vkb = []
    ...
    return jsonify(message)
########################################################################################################################
@app.route('/vkb_suub', methods=['GET'])
@cross_origin()
def vkb_suub():
    vkb_init = get_vkb.get_vkb(access_token)
    suub_vkb = []
    ...
    return jsonify(message)
########################################################################################################################
@app.route('/line_nkua', methods=['GET'])
@cross_origin()
def line_nkua():
    line_init = get_line.get_line(access_token)
    nkua_line = []
    for n in line_init:
        result1 = patients_filter.patients_filter([n], patients_lists.list_nkua)
        if (result1 == True):
    ...
    return jsonify(message)
########################################################################################################################
@app.route('/line_fism', methods=['GET'])
@cross_origin()
def line_fism():
    line_init = get_line.get_line(access_token)
    fism_line = []
    ...
    return jsonify(message)
########################################################################################################################
@app.route('/line_suub', methods=['GET'])
@cross_origin()
def line_suub():
    line_init = get_line.get_line(access_token)
    suub_line = []
...
def sa():
    ...
    return jsonify(message)
#####################################################################################################################>
@app.route('/sa_nkua', methods=['GET'])
@cross_origin()
def sa_nkua():
    sa=get_sa.get_sa(access_token)
    message = []
    for n in sa:
        if (n['organization']=='NKUA'):
            newMessage = {
                'Email_user': l['email'],
                'source': n["source"],
    ...
    return jsonify(message)
#####################################################################################################################>
@app.route('/sa_fism', methods=['GET'])
@cross_origin()
def sa_fism():
    sa=get_sa.get_sa(access_token)
    message = []
    for n in sa:
        if (n['organization']=='FISM'):
            newMessage = {
                'Email_user': l['email'],
                'source': n["source"],
                'organization': n['organization'],
    ...
    return jsonify(message)
#####################################################################################################################>
@app.route('/sa_suub1', methods=['GET'])
@cross_origin()
def sa_suub():
    sa=get_sa.get_sa(access_token)
    message = []
    for n in sa:
        if (n['organization']=='SUUB'):
            newMessage = {
                'Email_user': l['email'],
                'source': n["source"],
                'organization': n['organization'],
