...
#changed after appending Dirk's rules for MLN Reasoning(s)
response = requests.get('http://localhost:5545/mlnreas2')
# response = requests.get('http://localhost:5545/reasoning12') #, auth=('magneto','M4gnetoDS1'))#Check individually each rule/jar
...
def infer():
    ...
    for i in range(len(dataL)):
        ...
        sub = repr(dataL[i]['name'] + ' ' + dataL[i]['lname'])  # sub = repr(sub2+sub1)
        pre = repr('isSuspectMurder for ' + dataL[i]['inc'])  # pre = repr(ruleList[1])
        obj = repr(dataL[i]['confid'])  # obj = repr(ruleList[2])
