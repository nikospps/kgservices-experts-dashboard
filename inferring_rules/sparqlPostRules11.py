...
response = requests.get('http://localhost:5545/mlnreas')
# response = requests.get('http://localhost:5545/reasoning11') #, auth=('magneto','M4gnetoDS1'))#Check individually each rule/jar
...
def infer():
    ...
    for i in range(len(dataL)):
        ...
        pre = repr('isSuspect for ' + dataL[i]['inc'])  # pre = repr(ruleList[1])
